...
def mock_setup_entry():
    ...
    with patch("custom_components.frank_energie.async_setup_entry", return_value=True) as mock_setup:
        yield mock_setup
...
def mock_auth_exception():
    ...
    with patch("custom_components.frank_energie.config_flow.FrankEnergie") as mock_api:
        api_instance = mock_api.return_value
        api_instance.__aenter__.return_value = api_instance
        api_instance.login = AsyncMock(side_effect=ConnectionException("connection_error"))
        yield mock_api
...
def aioclient_responses(aioclient_mock, socket_enabled):
    ...
    aioclient_mock.post("https://frank-graphql-prod.graphcdn.app/", side_effect=next_response)

...
from homeassistant.const import CONF_ACCESS_TOKEN, CONF_PASSWORD, CONF_TOKEN, CONF_USERNAME, Platform
...
entry.options: bevat de gegevens die via een options flow zijn aangepast/nageleverd."""
    ...
    async def _get_site_reference_and_title(self,
        ...
        """Fetch site reference and human-readable title, with token renewal and credential fallback."""
        _LOGGER.debug("Getting site reference and title for coordinator")

        # Fetch UserSites with a three-step auth recovery chain:
        # 1. Try directly; 2. Token renewal; 3. Re-login with stored credentials
        try:
            user_sites_data = await coordinator.api.UserSites()
        except AuthException:
            _LOGGER.warning("AuthException during setup — attempting token renewal")
            try:
                ...
                if not username or not password:
                    _LOGGER.error("No stored credentials for auto re-login — manual reauth required")
                    raise ConfigEntryAuthFailed(
                        "Authentication failed and no stored credentials available"
                    ) from renew_err
                try:
                    auth = await coordinator.api.login(username, password)
                    self.hass.config_entries.async_update_entry(
                        self.entry,
                        data={
                            **self.entry.data,
                    ...
                    _LOGGER.info("Auto re-login successful during setup")
                    user_sites_data = await coordinator.api.UserSites()
                except Exception as login_err:
                    _LOGGER.error("Auto re-login failed: %s — manual reauth required", login_err)
                    raise ConfigEntryAuthFailed(
